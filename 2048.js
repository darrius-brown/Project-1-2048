...
function rightMove(lastColumn) {
    let right2 = document.getElementById(lastColumn - 1)
    let right1 = document.getElementById(lastColumn - 2)
    let right0 = document.getElementById(lastColumn - 3)

        if(occupied.includes(lastColumn - 1)){ 
            if(occupied.includes(lastColumn)) {
                let mergeRight1 = document.getElementById(lastColumn)
                if(right2.innerHTML === mergeRight1.innerHTML && merged.includes(mergeRight1.id) === false)
                    merge(right2, mergeRight1) 
                    fixOccupied(right2)
                } else {
                    moveSquare(lastColumn, right2)
                    fixOccupied(right2)
                }
        }
        if(occupied.includes(lastColumn - 2)){
            if(occupied.includes(lastColumn - 1)) {
                let mergeRight2 = document.getElementById(lastColumn - 1)
        ...
        if(occupied.includes(lastColumn - 3)){
            if(occupied.includes(lastColumn - 2)) {
                let mergeRight4 = document.getElementById(lastColumn - 2)
            ...
            fixOccupied(right0)
            if(occupied.includes(lastColumn - 1)) {
                let mergeRight5 = document.getElementById(lastColumn - 1)
            ...
            fixOccupied(right0)
            if(occupied.includes(lastColumn)) {
                let mergeRight6 = document.getElementById(lastColumn)
                if(right0.innerHTML === mergeRight6.innerHTML && merged.includes(mergeRight6.id) === false)
                merge(right0, mergeRight6)
                fixOccupied(right0)
        } else{
            moveSquare(lastColumn, right0)
            fixOccupied(right0)
        }           
}}}}
function leftMove(lastColumn) {
    let left2 = document.getElementById(lastColumn + 1)
    let left1 = document.getElementById(lastColumn + 2)
    let left0 = document.getElementById(lastColumn + 3)
    if(occupied.includes(lastColumn + 1)){
        if(occupied.includes(lastColumn)) {
            let mergeLeft1 = document.getElementById(lastColumn)
            if(left2.innerHTML === mergeLeft1.innerHTML && merged.includes(mergeLeft1.id) === false)
                merge(left2, mergeLeft1) 
                fixOccupied(left2)
        } else {
            moveSquare(lastColumn, left2)
            fixOccupied(left2)
        }
    }
    ...
    if(occupied.includes(lastColumn + 2)){
        if(occupied.includes(lastColumn + 1)) {
            let mergeLeft2 = document.getElementById(lastColumn + 1)
    ...
    if(occupied.includes(lastColumn + 3)){
        if(occupied.includes(lastColumn + 2)) {
            let mergeLeft4 = document.getElementById(lastColumn + 2)
        ...
        fixOccupied(left0)
        if(occupied.includes(lastColumn + 1)) {
            let mergeLeft5 = document.getElementById(lastColumn + 1)
        ...
        fixOccupied(left0)
        if(occupied.includes(lastColumn)) {
            let mergeLeft6 = document.getElementById(lastColumn)
            if(left0.innerHTML === mergeLeft6.innerHTML && merged.includes(mergeLeft6.id) === false)
                merge(left0, mergeLeft6) 
                fixOccupied(left0)
    } else{
        moveSquare(lastColumn, left0)
        fixOccupied(left0)
    }           
}}}}
function upMove(lastColumn) {
    let up2 = document.getElementById(lastColumn + 4)
    let up1 = document.getElementById(lastColumn + 8)
    let up0 = document.getElementById(lastColumn + 12)
    if(occupied.includes(lastColumn + 4)){
        if(occupied.includes(lastColumn)) {
            let mergeUp1 = document.getElementById(lastColumn)
            if(up2.innerHTML === mergeUp1.innerHTML && merged.includes(mergeUp1.id) === false)
                merge(up2, mergeUp1) 
                fixOccupied(up2)
            } else {
                moveSquare(lastColumn, up2)
                fixOccupied(up2)
            }
    }

    if(occupied.includes(lastColumn + 8)){
        if(occupied.includes(lastColumn + 4)) {
            let mergeUp2 = document.getElementById(lastColumn + 4)
            if(up1.innerHTML === mergeUp2.innerHTML && merged.includes(mergeUp2.id) === false)
                merge(up1, mergeUp2) 
                fixOccupied(up1)
    } else {
        moveSquare(lastColumn + 4, up1)
        fixOccupied(up1)
        if(occupied.includes(lastColumn)) {
            let mergeUp3 = document.getElementById(lastColumn)
            if(up1.innerHTML === mergeUp3.innerHTML && merged.includes(mergeUp3.id) === false)
                merge(up1, mergeUp3) 
                fixOccupied(up1)
        } else {
            moveSquare(lastColumn, up1)
            fixOccupied(up1)
        }
    }
    }

    if(occupied.includes(lastColumn + 12)){
        if(occupied.includes(lastColumn + 8)){
            let mergeUp4 = document.getElementById(lastColumn + 8)
            if(up0.innerHTML === mergeUp4.innerHTML && merged.includes(mergeUp4.id) === false)
                merge(up0, mergeUp4) 
                fixOccupied(up0)
    } else {
        moveSquare(lastColumn + 8, up0)
        fixOccupied(up0)
        if(occupied.includes(lastColumn + 4)) {
            let mergeUp5 = document.getElementById(lastColumn + 4)
            if(up0.innerHTML === mergeUp5.innerHTML && merged.includes(mergeUp5.id) === false)
                merge(up0, mergeUp5) 
                fixOccupied(up0)
    } else{
        moveSquare(lastColumn + 4, up0)
        fixOccupied(up0)
        if(occupied.includes(lastColumn)) {
            let mergeUp6 = document.getElementById(lastColumn)
            if(up0.innerHTML === mergeUp6.innerHTML && merged.includes(mergeUp6.id) === false)
                merge(up0, mergeUp6) 
                fixOccupied(up0)
    } else{
        moveSquare(lastColumn, up0)
        fixOccupied(up0)
    }           
}}}}
function downMove(lastColumn) {
    let down2 = document.getElementById(lastColumn - 4)
    let down1 = document.getElementById(lastColumn - 8)
    let down0 = document.getElementById(lastColumn - 12)
    if(occupied.includes(lastColumn - 4)){
        if(occupied.includes(lastColumn)) {
            let mergeDown1 = document.getElementById(lastColumn)
            if(down2.innerHTML === mergeDown1.innerHTML && merged.includes(mergeDown1.id) === false)
                merge(down2, mergeDown1) 
                fixOccupied(down2)
        } else {
            moveSquare(lastColumn, down2)
            fixOccupied(down2)
        }
    }
